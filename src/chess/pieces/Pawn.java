	...
	public boolean[][] possibleMoves() {
		...
		if (getColor() == Color.WHITE) {
			...
			if (position.getRow() == 3) {
				...
				if (getBoard().positionExists(left) && hasOpponentPiece(left) && (getBoard().piece(left) == chessMatch.getEnPassantVulnerable())) {
					possibleMoves[left.getRow() - 1][left.getColumn()] = true;
				}
				Position right = new Position(position.getRow(), position.getColumn() + 1);
				if (getBoard().positionExists(right) && hasOpponentPiece(right) && (getBoard().piece(right) == chessMatch.getEnPassantVulnerable())) {
